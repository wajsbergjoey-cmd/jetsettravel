...
export function RecentTrips() {
  return (
    <section id="trips" className="bg-card py-24">
      <div className="wrap">
        <Reveal className="mb-11 flex flex-wrap items-baseline justify-between gap-[14px]">
        ...
        <div className="grid grid-cols-1 gap-[14px] min-[520px]:grid-cols-2 min-[900px]:grid-cols-4">
          {tiles.map((t, i) => (
            <Reveal key={t.label} delay={i * 90}>
              <figure className="tile-overlay group relative flex h-[240px] items-end overflow-hidden rounded-[3px] p-4 min-[900px]:h-[300px]">
                <img
                  src={t.url}
                ...
                />
                <figcaption className="relative z-[2] font-display text-[1rem] text-cream italic">
                  {t.label}
                </figcaption>
              </figure>
            </Reveal>
